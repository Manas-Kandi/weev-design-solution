...
export class DecisionTreeNode extends BaseNode {
  async execute(context: NodeContext): Promise<NodeOutput> {
    ...
    const sortedRules = evaluationMode === 'priority' 
  ...
  private async evaluateCondition(condition: string, context: string): Promise<boolean> {
    const evalPrompt = `
...
`;
  ...
  private async evaluateWithLLM(
    ...
    const rulesText = rules
      .map((r, i) => `${i + 1}. If "${r.condition}" then output "${r.outputPath}"`)
      .join('\n');
...
`;
    ...
    try {
      ...
      const result = response.candidates?.[0]?.content?.parts?.[0]?.text?.trim() || defaultPath;
      return result;
    } catch (error) {
